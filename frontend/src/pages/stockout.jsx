...
const stockout = () => {
  ...
  const fetchStockOutLogs = () => {
    axios.get('http://127.0.0.1:8000/stockOutLog/')
      .then(response => {
  ...
  const fetchStockOutLogItems = () => {
    axios.get('http://127.0.0.1:8000/stockOutLogItem/')
      .then(response => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const logResponse = await axios.post('http://127.0.0.1:8000/stockOutLog/', {
        dateCreated: '',
      });
  ...
  const handleDelete = (logId) => {
    axios.delete(`http://127.0.0.1:8000/stockOutLog/${logId}/`)
      .then(response => {
  ...
  const handleAddItem = (e) => {
    ...
    // Send POST request
    axios.post('http://127.0.0.1:8000/stockOutLogItem/', {
      logID: currentLog.id,
  ...
  const handleDeleteItem = (logId) => {
    axios.delete(`http://127.0.0.1:8000/stockOutLogItem/${logId}/`)
      .then(response => {
  ...
  const handleUpdateItem = (e) => {
    ...
    // Send POST request
    axios.put(`http://127.0.0.1:8000/stockOutLogItem/${currentItemID}/`, {
      logID: currentLog.id,
      productID: stockOutLogItem.productID,
      stockItemID: stockItemtoAdd.id,
      referenceNumber: stockOutLogItem.referenceNumber,
  ...
  const handleButtonClick = (newStatus) => {
    ...
    axios.put(`http://127.0.0.1:8000/stockOutLog/${currentLog.id}/`, {
      status: newStatus,
    })
      .then(response => {
    ...
    <div className='w-screen h-screen bg-cover bg-center flex font-poppins' style={{ backgroundImage: `url(${bg})` }}>
      ...
      <div className='w-[83.5vw] h-screen flex flex-col items-center z-50'>
        <div className='w-full h-[10vh] bg-white flex items-center justify-between px-[2vw] drop-shadow-xl'>
          <h1 className='text-[1.5vw] text-darkp font-medium tracking-tighter z-10'>Stock Out</h1>
        </div>
                        ...
                        {stockItems
                          .filter(stockItem => {
                            ...
                            return (
                              ...
                              matchingStock.productId === parseInt(stockOutLogItem.productID) &&
                              stockItem.displayedStock > 0
                            );
                          })
                          .map(stockItem => (
                  ...
                  <div className='w-full flex gap-5 justify-between'>
                    ...
                    <div className='w-[30%] flex flex-col justify-start gap-1'>
                      ...
                      >
                        ...
                        {stockItems
                          .filter(stock =>
                            stock.productID === parseInt(stockOutLogItem.productID) &&
                            stock.closedStock > 0
                          ) 
                          .map(stock => {
                            return (
